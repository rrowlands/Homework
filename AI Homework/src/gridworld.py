...
def parseOptions():
    ...
    optParser.add_option('-a', '--agent',action='store', metavar="A",
                         ...
                         help='Agent type (options are \'random\', \'value\', \'hardcoded\', and \'q\', default %default)')
...
if __name__ == '__main__':
    ...
    if opts.agent == 'value':
        ...
        class RandomAgent:
            ...
            def update(self, state, action, nextState, reward):
                pass
        a = RandomAgent()
    elif opts.agent == 'hardcoded':
        import hardcoded_mdp
        mdp = hardcoded_mdp.HardcodedMDP("R", hardcoded_mdp.EXAMPLE_MDP)
        env = hardcoded_mdp.HardcodedEnvironment(mdp)
        a = valueIterationAgents.ValueIterationAgent(mdp, opts.discount, opts.iters)
    else:
    ...
    if not opts.manual and opts.agent == 'hardcoded':
        if opts.valueSteps:
            print "Value Iterations. Note that q-values for iteration 0 are 0.0\n"

            import pprinttable
            from collections import namedtuple

            states = sorted(list(mdp.getStates()))
            Row = namedtuple('Row', ['i'] + ["V_%s" % state for state in states])
            rows = []

            QRow = namedtuple('QRow', ['i'] + ["Q_%s_%s" % (state, action)  for state in states for action in mdp.getPossibleActions(state)])
            qrows = []

            PRow = namedtuple('pRow', ['i'] + ["P_%s" % state for state in states])
            prows = []

            for i in range(opts.iters):
                tempAgent = valueIterationAgents.ValueIterationAgent(mdp, opts.discount, i)

                rows.append(Row(*[i] + [tempAgent.getValue(state) for state in states]))

                qrows.append(QRow(*[i+1] + [tempAgent.getQValue(state, action) for state in states for action in mdp.getPossibleActions(state)]))
                prows.append(PRow(*[i+1] + [tempAgent.getPolicy(state) for state in states]))

            pprinttable.pprinttable(rows)
            print
            pprinttable.pprinttable(qrows)
            print
            pprinttable.pprinttable(prows)
